    ...
    <div className="flex-1 flex flex-col min-w-0">
      <section className="p-4 sm:p-6 lg:p-8 xl:p-12 space-y-8 sm:space-y-12 pb-24 md:pb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="md:col-span-3 bg-surface-container-low p-4 sm:p-6 rounded-2xl sm:rounded-3xl flex flex-wrap items-center gap-3 sm:gap-4">
          ...
          <div className="bg-primary-container text-on-primary-container p-4 sm:p-6 rounded-2xl sm:rounded-3xl flex items-center justify-between">
            <div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-4 sm:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7 gap-3 sm:gap-5 lg:gap-6">
            {pagedChars.map((c, i) => (
              <div
                key={c.id}
                className={`group relative bg-surface-container-lowest rounded-2xl sm:rounded-3xl overflow-hidden transition-all duration-300 hover:translate-y-[-8px] hover:shadow-[0_24px_48px_-12px_rgba(70,72,212,0.12)] ${i === 0 ? "border-2 border-primary/20" : ""}`}
              >
                ...
                <div className="absolute inset-0 z-0 opacity-10 grayscale group-hover:grayscale-0 group-hover:opacity-30 transition-all duration-500 scale-110 group-hover:scale-100">
                  <Image
                    ...
                    src={discoverImages[i % discoverImages.length]}
                    alt={c.meaning}
                    fill
                    sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, (max-width: 1280px) 25vw, (max-width: 1536px) 16vw, 14vw"
                    loading="lazy"
                  />
                </div>
              </div>
