...
export function FlashcardsPanel({ initialDeckId, onBackToDecks, onStartQuiz }: FlashcardsPanelProps) {
  ...
  // Sync initialDeckId when parent changes it (e.g., user clicks "Study" from Decks tab)
  useEffect(() => {
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-12 pb-24 md:pb-8">
        <div className="space-y-2 mb-8">
          <div className="flex items-center gap-3">
            <button onClick={onBackToDecks} className="text-[0.625rem] sm:text-[0.6875rem] font-bold tracking-[0.1em] text-primary uppercase hover:underline">
              ← Back to Decks
            </button>
          </div>
          <h1 className="text-xl sm:text-2xl font-[family-name:var(--font-jakarta)] font-extrabold text-on-surface">Choose a Deck to Study</h1>
          <p className="text-sm text-on-surface-variant">Select a deck below, or go to the Decks tab to create one.</p>
        </div>
        {decks.length === 0 ? (
          <div className="text-center py-16">
            <span className="material-symbols-outlined text-5xl text-outline/40 block mb-4">collections_bookmark</span>
            <p className="text-on-surface-variant mb-4">No decks available yet.</p>
            <button onClick={onBackToDecks}
              className="px-6 py-3 bg-gradient-to-r from-primary to-secondary text-on-primary rounded-full font-bold text-sm hover:opacity-90 transition-opacity">
              Create a Deck
            </button>
          </div>
        ) : (
  ...
  if (currentIndex >= total && total > 0) {
    const accuracy = total > 0 ? Math.round((mastered / (mastered + again)) * 100) : 0;
    return (
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-12 flex flex-col items-center justify-center pb-24 md:pb-8 text-center space-y-6">
        ...
        <div className="flex gap-6 text-sm">
          ...
          <div className="text-center">
            <span className="text-2xl font-bold text-secondary block">{accuracy}%</span>
            <span className="text-on-surface-variant">Accuracy</span>
          </div>
        </div>

        {/* Workflow actions — guide user to next step */}
        <div className="flex flex-col sm:flex-row gap-3 mt-4">
          <button onClick={() => { setCurrentIndex(0); setMastered(0); setAgain(0); }}
            className="px-6 py-3 bg-surface-container-high text-on-surface rounded-full font-bold hover:bg-surface-container-highest transition-colors flex items-center justify-center gap-2">
            <span className="material-symbols-outlined text-[20px]">replay</span> Restart Deck
          </button>
          <button onClick={onStartQuiz}
            className="px-8 py-3 bg-gradient-to-r from-primary to-secondary text-white rounded-full font-bold shadow-xl shadow-primary/20 hover:opacity-90 transition-all flex items-center justify-center gap-2">
            <span className="material-symbols-outlined text-[20px]">quiz</span> Test Your Knowledge
          </button>
          <button onClick={() => { setSelectedDeckId(null); }}
            className="px-6 py-3 bg-surface-container-high text-on-surface rounded-full font-bold hover:bg-surface-container-highest transition-colors">
            Choose Another
          </button>
        </div>
      </div>
  ...
  if (!card) {
    return (
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-12 flex flex-col items-center justify-center pb-24 md:pb-8 space-y-4">
        <span className="material-symbols-outlined text-5xl text-outline/40">style</span>
        <p className="text-on-surface-variant text-sm">No cards in this deck.</p>
        <button onClick={() => setSelectedDeckId(null)} className="text-primary font-bold text-sm hover:underline">← Choose another deck</button>
      </div>
    );
  }
